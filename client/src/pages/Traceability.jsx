      ...
      <div className="card overflow-hidden">
        <div className="grid grid-cols-1 lg:grid-cols-[auto_1fr_auto] items-stretch">
          {source.photo_path ? (
            ...
            <div className="hidden lg:flex w-48 items-center justify-center bg-slate-50 dark:bg-slate-800 dark:text-slate-600 border-r border-slate-100 dark:border-slate-800">
              <FiCamera size={40} />
            </div>
          )}
          ...
          <div className="p-5 lg:border-l border-slate-100 dark:border-slate-800 flex flex-col items-center justify-center gap-4 bg-slate-50/50 dark:bg-slate-800/40">
            <div className="flex items-start justify-center gap-4">
              <div className="flex flex-col items-center">
                {qr ? (
                  ...
                  <div className="w-28 h-28 rounded-lg bg-white flex items-center justify-center text-[10px] text-slate-400 dark:text-slate-500 dark:bg-slate-800">QR unavailable</div>
                )}
              </div>
              <div className="flex flex-col items-center">
                {barcode ? (
                  ...
                  <div className="w-28 h-28 rounded-lg bg-white flex items-center justify-center text-[10px] text-slate-400 dark:text-slate-500 dark:bg-slate-800">Barcode unavailable</div>
                )}
              </div>
            </div>
        ...
        <section className="card p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-sm text-slate-800 dark:text-slate-100">Additional photos</h3>
            </div>
            <span className="text-xs text-slate-400 dark:text-slate-500">{(source.photos || []).length} photo(s)</span>
          </div>
          ...
          <section className="card overflow-hidden">
            <header className="card-header">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-sm text-slate-800 dark:text-slate-100">Status change history</h2>
              </div>
              <span className="text-xs text-slate-400 dark:text-slate-500">{statusRows.length} events</span>
            </header>

            {/* Current state summary */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 p-4 bg-slate-50/60 border-b border-slate-100 dark:border-slate-800 dark:bg-slate-800/40">
            ...
            {statusRows.length === 0 && (
              <p className="text-sm text-slate-400 dark:text-slate-500 p-5">No status changes recorded yet.</p>
            )}
          </section>
          ...
          <section className="card overflow-hidden">
            <header className="card-header">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-sm text-slate-800 dark:text-slate-100">Full change log</h2>
              </div>
              <span className="text-xs text-slate-400 dark:text-slate-500">audit trail</span>
            </header>
            <div className="table-wrap">
              <table className="data-table">
                <thead>
                ...
                <tbody>
                  {(source.history || []).map((h) => (
                    <tr key={h.id}>
                      <td className="whitespace-nowrap text-slate-500 dark:text-slate-400 dark:text-slate-500">{formatDate(h.changed_at)}</td>
                      <td><span className="badge-slate capitalize">{h.change_type}</span></td>
                      <td className="font-mono text-slate-700 dark:text-slate-200">{h.field_changed}</td>
                      <td className="text-slate-400 dark:text-slate-500">{h.previous_value || '—'}</td>
                      <td className="font-medium text-slate-700 dark:text-slate-200">{h.new_value || '—'}</td>
                      <td className="text-slate-500 dark:text-slate-400 dark:text-slate-500">{h.changed_by_name || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {(source.history || []).length === 0 && (
              <p className="text-sm text-slate-400 dark:text-slate-500 p-5">No changes yet.</p>
            )}
          </section>
...
function Row({ h }) {
  ...
  return (
    <tr>
      <td className="whitespace-nowrap text-slate-500 dark:text-slate-400 dark:text-slate-500">{formatDate(h.date)}</td>
      <td>
        {h.field === 'conditioning_status' ? <Chip value={h.next} /> : cols.conditioning_status ?? '—'}
      </td>
      <td className="text-slate-600 dark:text-slate-300 dark:text-slate-600">{cols.storage_facility_unit || <span className="text-slate-300 dark:text-slate-600">—</span>}</td>
      <td className="font-mono text-slate-600 dark:text-slate-300 dark:text-slate-600">{cols.capsule_no_id || <span className="text-slate-300 dark:text-slate-600">—</span>}</td>
      <td className="font-mono text-slate-600 dark:text-slate-300 dark:text-slate-600">{cols.concrete_drum_no || <span className="text-slate-300 dark:text-slate-600">—</span>}</td>
      <td className="text-slate-500 dark:text-slate-400 dark:text-slate-500">{h.changed_by || '—'}</td>
    </tr>
  );
}
...
function Stat({ label, value }) {
  return (
    <div>
      ...
      <p className="text-sm font-semibold text-slate-800 dark:text-slate-100 capitalize truncate">{value}</p>
    </div>
  );
}
...
function HeroItem({ label, value, mono }) {
  return (
    <div className="min-w-0">
      <p className="text-[11px] uppercase tracking-wide text-slate-400 dark:text-slate-500">{label}</p>
      <p className={`text-sm font-semibold text-slate-800 dark:text-slate-100 truncate ${mono ? 'font-mono' : ''}`}>{value || '—'}</p>
    </div>
  );
}
    ...
    <div className="space-y-5">
      {groups.map((g) => (
        <section key={g.title} className="card overflow-hidden">
          <header className="card-header">
            <h3 className="font-semibold text-sm text-slate-800 dark:text-slate-100">{g.title}</h3>
          </header>
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 p-5">
            {g.items.filter(([, v]) => v !== null && v !== undefined && v !== '').map(([label, value, mono], i) => (
              <div key={i} className="flex items-center justify-between py-2 border-b border-slate-50 dark:border-slate-800">
                ...
                <dd className={`text-sm font-medium text-slate-800 dark:text-slate-100 text-right ${mono ? 'font-mono' : ''}`}>{value}</dd>
              </div>
            ))}
          </dl>
        </section>
    ...
    <section className="card overflow-hidden">
      <header className="card-header">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-sm text-slate-800 dark:text-slate-100">Measurements</h3>
        </div>
      </header>
      {items.length === 0 ? (
        <p className="text-sm text-slate-400 dark:text-slate-500 p-5">None recorded</p>
      ) : (
        <div className="p-4 space-y-3">
          {items.map((m) => (
            <div key={m.id} className="flex items-center justify-between text-sm border-b border-slate-50 dark:border-slate-800 pb-2">
              <div>
                <p className="text-slate-600 dark:text-slate-300 dark:text-slate-600">{formatDate(m.measurement_date)}</p>
                <p className="text-[11px] text-slate-400 dark:text-slate-500">{m.instrument_used || '—'}</p>
              </div>
              <p className="font-mono font-semibold text-slate-800 dark:text-slate-100">{m.dose_rate_at_1m} mSv/h @1m</p>
            </div>
          ))}
        </div>
      )}
    ...
    <section className="card overflow-hidden">
      <header className="card-header">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-sm text-slate-800 dark:text-slate-100">Leak tests</h3>
        </div>
      </header>
      {items.length === 0 ? (
        <p className="text-sm text-slate-400 dark:text-slate-500 p-5">None recorded</p>
        ...
        <div className="p-4 space-y-3">
          {items.map((t) => (
            <div key={t.id} className="flex items-center justify-between text-sm border-b border-slate-50 dark:border-slate-800 pb-2">
              <div>
                <p className="text-slate-600 dark:text-slate-300 dark:text-slate-600">{formatDate(t.leak_test_date)}</p>
                <p className="text-[11px] text-slate-400 dark:text-slate-500">{t.leak_test_method || t.instrument_used || '—'}</p>
              </div>
