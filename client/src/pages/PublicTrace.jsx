    ...
    return (
      <div className="min-h-screen bg-slate-100 dark:bg-slate-950 flex items-center justify-center p-6">
        <div className="card p-8 text-center max-w-sm">
          ...
          <h1 className="font-semibold text-slate-800 dark:text-slate-100">Source not found</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-1">This code does not match a registered DSRS record.</p>
  ...
  if (!source) {
    return (
      <div className="min-h-screen bg-slate-100 dark:bg-slate-950 flex items-center justify-center">
        <span className="w-6 h-6 border-2 border-slate-300 dark:border-slate-600 border-t-brand-600 rounded-full animate-spin" />
      </div>
    );
  }
        ...
        <header className="card overflow-hidden">
          <div className="flex items-start justify-between flex-wrap gap-3">
            <div>
              ...
              <h1 className="text-xl font-bold text-slate-900 dark:text-slate-50 mt-0.5">
          ...
          {source.photo_path && (
            <img src={`/uploads/${source.photo_path}`} alt="Source" className="w-full h-48 object-cover border-t border-slate-100 dark:border-slate-800 mt-4" />
          )}
          ...
          <div className="flex items-center flex-wrap gap-2 mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
          ...
          {(qr || barcode) && (
            <div className="flex items-center justify-center gap-6 mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/40 rounded-lg p-3">
              ...
              <p className="text-[11px] text-slate-400 dark:text-slate-500 max-w-[140px]">Scanning this source's QR opens this public record.</p>
            </div>
          )}
        </header>
        ...
        <section className="card overflow-hidden">
          <header className="card-header">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-sm text-slate-800 dark:text-slate-100">Lifecycle history</h2>
            </div>
            <span className="text-xs text-slate-400 dark:text-slate-500">{statusChanges.length} milestone(s), {(source.history || []).length} audit event(s)</span>
          </header>
          {(source.history || []).length === 0 ? (
            <p className="text-sm text-slate-400 dark:text-slate-500 p-5">No recorded events.</p>
          ) : (
            <ol className="p-5 space-y-0">
              {(source.history || []).slice().reverse().map((h, i) => (
                <li key={h.id} className="relative pl-6 pb-4 border-l-2 border-slate-100 dark:border-slate-800 ml-2 last:pb-0">
                  <span className="absolute -left-[7px] top-1 w-3 h-3 rounded-full bg-brand-500 ring-4 ring-brand-50 dark:ring-brand-500/20" />
                  <p className="text-xs text-slate-400 dark:text-slate-500">{formatDate(h.changed_at)} · by {h.changed_by_name || 'system'}</p>
                  <p className="text-sm text-slate-700 dark:text-slate-200">
                    <span className="capitalize font-medium">{h.change_type}</span>
                    {h.field_changed ? (
                      <>
                        {' '}— <span className="font-mono text-slate-400 dark:text-slate-500">{h.field_changed}</span>:
                        {h.previous_value ? <s className="text-slate-400 dark:text-slate-500 mx-1">{h.previous_value}</s> : null}
                        <span className="text-slate-800 dark:text-slate-100 font-medium">{h.new_value}</span>
                      </>
                    ) : null}
                  </p>
                </li>
        ...
        <div className="p-4 space-y-3">
          {items.map((m) => (
            <div key={m.id} className="flex items-center justify-between text-sm border-b border-slate-50 dark:border-slate-800 pb-2">
              <div>
                <p className="text-slate-600 dark:text-slate-300">{formatDate(m.measurement_date)}</p>
        ...
        <div className="p-4 space-y-3">
          {items.map((t) => (
            <div key={t.id} className="flex items-center justify-between text-sm border-b border-slate-50 dark:border-slate-800 pb-2">
              <div>
                <p className="text-slate-600 dark:text-slate-300">{formatDate(t.leak_test_date)}</p>
